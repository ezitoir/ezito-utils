...
function mergeArray ( objectArray ){
    if(!(objectArray instanceof Array)) throw new Error(' objectArray param must be array type');
    let resultObject = {};
    for (const iterator of objectArray) {
        resultObject = merge(resultObject , iterator);  
    }
    return resultObject ;
}
